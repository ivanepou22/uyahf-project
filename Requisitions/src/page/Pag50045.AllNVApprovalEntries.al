...
/// </summary>
page 50045 "All NV Approval Entries"
{
    ...
    SourceTableView = SORTING("Sequence No.", "Table ID", "Document Type", "Document No.", "Date-Time Sent for Approval")
                      ORDER(descending) where(Status = filter(Open | Approved | Created));
    ...
    layout
    {
        area(content)
        {
            repeater(Control1)
            {
                ...
                field("Document Type1"; "Document Type1")
                {
    ...
    begin
        if Status in [Status::Created, Status::Open, Status::Approved, Status::Canceled, Status::Rejected] then begin
